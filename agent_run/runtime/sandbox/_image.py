...
def render_dockerfile(spec: AgentSpec, *, has_skills: bool) -> str:
    ...
    lines = [
        "# Generated by agent-run (runtime/sandbox/_image.py) — do not edit.",
        ...
        f"COPY agent_run {TOOLKIT_DIR}/agent_run",
    ]
    if has_skills:
    ...
    lines += [
        ...
        f"    AGENT_RUN_WORKSPACE_ROOT={WORKSPACE_ROOT} \\",
        f"    AGENT_RUN_BAKED_SPEC={TOOLKIT_DIR}/spec.json \\",
        "    PYTHONUNBUFFERED=1 \\",
        "    PYTHONDONTWRITEBYTECODE=1",
        f"WORKDIR {WORKSPACE_ROOT}",
        "EXPOSE 8080",
        'CMD ["python", "-m", "agent_run.runtime.sandbox.worker"]',
        "",
    ]
    return "\n".join(lines)
...
def _toolkit_package_dir() -> Path:
    ...
    return Path(agent_run.__file__).resolve().parent
...
def stage_build_context(spec: AgentSpec) -> tuple[Path, str]:
    ...
    """
    stage = Path(tempfile.mkdtemp(prefix="agent-run-image-"))
    shutil.copytree(
        _toolkit_package_dir(), stage / "agent_run",
...
def image_uri(image_repo: str, spec_name: str, digest: str) -> str:
    """``<repo>/agent-run-<name>:<digest>`` — the tag is the content digest of the build context."""
    import re

    slug = re.sub(r"[^a-z0-9._-]", "-", spec_name.lower()).strip("-.") or "agent"
    if not slug.startswith("agent-run-"):
        slug = f"agent-run-{slug}"
