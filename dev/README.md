...
[`Dockerfile`](Dockerfile) mirrors the **install contract** of the sandbox image `sandbox.deploy` builds
...
The production image itself is generated by `runtime/sandbox/_image.py` from the same contract.
...
## Relation to `sandbox.deploy`

`sandbox.deploy` installs `AgentSpec.packages` into the sandbox image via the same requirements path this
...
| [`live_resume_probe.py`](live_resume_probe.py) | — | Claude resume regression: stop a turn with a pending background task, then reply and check the task survived. Runs locally by default; set `ENGINE`/`PROJECT`/`LOCATION` to run it against a sandbox engine you deployed (and own the teardown of) |
| [`live_two_turn_probe.py`](live_two_turn_probe.py) | — | two turns on ONE session over the GCS event stream: the second turn must stream its own events and terminal result rather than replay the first's, `history()` must return both, and event latency stays under ~5 s. Throwaway cold engine, teardown in `finally` |
