...
class Session(Protocol):
    ...
    def run(
        ...
        *,
        ...
        ``config`` is this turn's :class:`~harness_run.config.TurnConfig` — a
    ...
    def send(
        ...
        **Idle** (e.g. after a ``needs_input`` pause): starts a new turn that resumes via
        ...
        per-turn :class:`~harness_run.config.TurnConfig`; the SESSION config cannot
        ...
        * ``interrupt=True``: the model is interrupted first, then continues from the message
        ...
        :class:`~harness_run.control.ControlUnavailable`; a second concurrent turn
        is never started.
        """
        ...
    ...
    async def exec(
        ...
        :data:`~harness_run.control.EXEC_DEFAULT_TIMEOUT_S`; a command that runs
        ...
        ended, it raises :class:`~harness_run.control.ControlUnavailable`. The
...
class Engine(Protocol):
    ...
    def start_session(self, config: SessionConfig | None = None) -> Session:
        ...
        ``config`` binds the session's :class:`~harness_run.config.SessionConfig`
