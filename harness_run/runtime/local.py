...
class LocalSession:
    ...
    The session's :class:`~harness_run.config.SessionConfig` (bound at
    ``engine.start_session(config=...)``) overlays the engine's spec for every turn of
    this session; a per-turn :class:`~harness_run.config.TurnConfig` overlays
    ...
    def run(
        ...
        *,
        ...
        :class:`~harness_run.config.TurnConfig` overlay (invocation knobs only).
        *hooks* are Claude Agent SDK hook callbacks for this turn
        (``{HookEvent: [HookMatcher, ...]}``); see :meth:`~harness_run.runtime.base.Session.run`.
    ...
    def send(
        ...
        *,
        ...
        same reason. ``config`` is a per-turn :class:`~harness_run.config.TurnConfig`;
        ...
        :class:`Run` is returned (see :meth:`~harness_run.runtime.base.Session.send`):
    ...
    async def exec(
        ...
        raises :class:`~harness_run.control.ControlUnavailable`, so code written
...
class LocalEngine:
    ...
    def __init__(
        ...
        self.spec = spec
        root = Path(workdir) if workdir else Path(tempfile.mkdtemp(prefix="harness-run-"))
