...
``sandbox.deploy`` turns a spec into a container image the platform runs as a custom
...
def render_dockerfile(spec: AgentSpec, *, has_skills: bool) -> str:
    ...
    lines = [
        "# Generated by harness-run (runtime/sandbox/_image.py) — do not edit.",
        ...
        f"COPY harness_run {TOOLKIT_DIR}/harness_run",
    ]
    if has_skills:
    ...
    lines += [
        ...
        f"    HARNESS_RUN_WORKSPACE_ROOT={WORKSPACE_ROOT} \\",
        f"    HARNESS_RUN_BAKED_SPEC={TOOLKIT_DIR}/spec.json \\",
        "    PYTHONUNBUFFERED=1 \\",
        "    PYTHONDONTWRITEBYTECODE=1",
        f"WORKDIR {WORKSPACE_ROOT}",
        "EXPOSE 8080",
        'CMD ["python", "-m", "harness_run.runtime.sandbox.worker"]',
        "",
    ]
    return "\n".join(lines)
...
def _toolkit_package_dir() -> Path:
    ...
    return Path(harness_run.__file__).resolve().parent
...
def stage_build_context(spec: AgentSpec) -> tuple[Path, str]:
    ...
    """
    stage = Path(tempfile.mkdtemp(prefix="harness-run-image-"))
    shutil.copytree(
        _toolkit_package_dir(), stage / "harness_run",
...
def image_uri(image_repo: str, spec_name: str, digest: str) -> str:
    """``<repo>/harness-run-<name>:<digest>`` — the tag is the content digest of the build context."""
    import re

    slug = re.sub(r"[^a-z0-9._-]", "-", spec_name.lower()).strip("-.") or "agent"
    if not slug.startswith("harness-run-"):
        slug = f"harness-run-{slug}"
...
def build_and_push(context: Path, uri: str, *, log: Any = None) -> str:
    ...
    if shutil.which(DOCKER) is None:
        raise RuntimeError(
            "sandbox.deploy builds the sandbox image with the Docker CLI, which is not on "
