...
``harness-run-gcp-setup --project <id>`` makes a GCP project ready to run this toolkit's
...
* the **model service account** (``harness-run-model@``): the identity whose one-hour tokens the
...
DEFAULT_REPO_ID = "harness-run"  # must mirror _image.DEFAULT_REPO_ID (deploy's default image_repo)
...
MODEL_PREDICT_ROLE_ID = "harnessRunPredict"  # custom role: model calls only (kept from the earlier model)
...
def predict_role_name(project: str) -> str:
    """Full resource name of the project's ``harnessRunPredict`` custom role."""
...
class GcpApi:
    ...
    def create_repository(self, location: str, repo_id: str) -> None:
        op = self.request(
            ...
            json_body={"format": "DOCKER", "description": "harness-run sandbox images"},
        )
        ...
        items.append(
            Item(
                ...
                fix=lambda i=model_email.split("@")[0]: api.create_service_account(
                    i, "harness-run model identity (Vertex model calls only)"
                ),
            )
        )
        ...
        items.append(
            Item(
                ...
                fix=lambda: api.create_role(
                    MODEL_PREDICT_ROLE_ID, "harness-run model identity: model calls only", MODEL_PREDICT_PERMISSIONS
                ),
            )
        )
            ...
            items.append(
                Item(
                    ...
                    fix=lambda i=op_email.split("@")[0]: api.create_service_account(
                        i, "harness-run operator (control plane)"
                    ),
                )
            )
...
def verify(api: GcpApi, cfg: Settings, items: Sequence[Item]) -> tuple[bool, list[Item]]:
    ...
    spec = AgentSpec(
        harness="claude-code",
        name=f"harness-run-setup-verify-{user}",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="harness-run-gcp-setup",
        description=(
            "Make a GCP project ready for harness-run's sandbox backend: audit, "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print_report(items, f"harness-run GCP setup — {cfg.project} ({cfg.location})")
