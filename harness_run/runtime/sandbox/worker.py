...
* ``/token``   — ``{turn_id, model_token: {access_token, expires_at}}``: replaces the model
  ...
  ``HARNESS_RUN_METADATA_PORT``) speaks the GCE metadata protocol for exactly one thing — the running
...
BOOT = time.time()
WORKSPACE_ROOT = os.environ.get("HARNESS_RUN_WORKSPACE_ROOT", "/workspace")
...
BAKED_SPEC_PATH = os.environ.get("HARNESS_RUN_BAKED_SPEC", "/opt/toolkit/spec.json")
...
def load_baked_spec(path: str = BAKED_SPEC_PATH) -> Any:
    """The spec baked into this image at deploy (``HARNESS_RUN_BAKED_SPEC``)."""
...
def find_baked_skills(spec_path: str = BAKED_SPEC_PATH) -> Path | None:
    ...
    pkg = Path(harness_run.__file__).resolve().parent
...
class Worker:
    ...
    def __init__(
        ...
        *,
        ...
        # (``0`` → an ephemeral port; tests). ``None`` → ``HARNESS_RUN_METADATA_PORT`` or 8081.
        ...
        # and how often (``None`` → the HARNESS_RUN_RESOURCE_SAMPLE_S env / 20 s).
    ...
    def _ensure_metadata_server(self) -> str:
        with self._lock:
            if self._metadata is None:
                ...
                if port is None:
                    port = int(os.environ.get("HARNESS_RUN_METADATA_PORT", str(DEFAULT_METADATA_PORT)))
...
class _MetadataHandler(BaseHTTPRequestHandler):
    ...
    server_version = "harness-run-metadata/1"
...
class _MetadataServer(ThreadingHTTPServer):
    ...
    def __init__(self, worker: Worker, port: int) -> None:
        ...
        threading.Thread(target=self.serve_forever, name="harness-run-metadata", daemon=True).start()


class Handler(BaseHTTPRequestHandler):
    server_version = "harness-run-sandbox-worker/1"
...
def main() -> None:
    ...
    sys.stderr.write(f"harness-run sandbox worker on :{port} (uid {os.getuid()})\n")
    srv.serve_forever()
