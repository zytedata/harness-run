"""Offline tests for the GCP project-setup tool (ratk-gcp-setup), sandbox edition.
...
def test_sandbox_agent_email_and_repo_helpers():
    ...
    assert ps.repo_resource("p", "us-central1", "ratk") == "projects/p/locations/us-central1/repositories/ratk"
    assert ps.image_repo_uri("p", "us-central1", "ratk") == "us-central1-docker.pkg.dev/p/ratk"


def test_defaults_mirror_backend_deploy_defaults():
    # backend.deploy defaults to gs://<project>-agent-output and the `ratk` repo; the setup
    # tool must create exactly those or a plain deploy() won't find them.
    from remote_agent_toolkit.runtime.gemini import _image, model_token
...
def test_settings_emails_and_overrides():
    ...
    assert cfg.model_email() == "ratk-model@p.iam.gserviceaccount.com"
...
MODEL = f"serviceAccount:ratk-model@{PROJECT}.iam.gserviceaccount.com"
...
def test_audit_ready_project_is_all_ok():
    ...
    api.create_service_account("ratk-model", "x")
...
def test_verify_deploys_with_the_projects_repo_and_model_sa(monkeypatch):
    ...
    assert seen["image_repo"] == f"{LOCATION}-docker.pkg.dev/{PROJECT}/ratk"
