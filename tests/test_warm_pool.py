...
from harness_run.runtime.sandbox.provider import SandboxError
...
def test_fill_pool_creates_ready_sandboxes_and_records_them():
    ...
    for e in entries:
        ...
        assert provider.sandboxes[e.sandbox]["display_name"].startswith("harness-run-g-")
...
def test_delete_releases_every_sandbox_and_retires_all_versions(monkeypatch):
    ...
    engine.fill_pool(2)
    orphan = provider.create(engine.resource, ttl_s=60, display_name="harness-run-g-orphan")  # a crashed client's
    foreign = provider.create(old, ttl_s=60, display_name="harness-run-other-x")
    monkeypatch.setattr(backend.time, "sleep", lambda s: None)
...
def test_delete_of_engine_x_leaves_the_pool_of_engine_x_suffixed_alone(monkeypatch):
    ...
    # display-name prefix ``harness-run-x-`` matched them. The sweep goes by template now.
    provider = FakeSandboxProvider()
    ...
    lookalike = provider.create(pinned.resource, ttl_s=60, display_name="harness-run-x-deadbeef")
    provider.sandboxes[lookalike.name]["display_name"] = "harness-run-x-b040d27a"
    templateless = provider.create(legacy.resource, ttl_s=60, display_name="harness-run-x-0123abcd")
...
def test_retire_templates_gives_up_after_the_timeout(monkeypatch):
    ...
    provider.create(other, ttl_s=60, display_name="harness-run-g-busy")  # blocks the template delete
    monkeypatch.setattr(backend.time, "sleep", lambda s: None)
...
def test_get_engine_handle_shares_the_pool_of_the_deploying_process():
    ...
    other = backend.SandboxEngine(template=deploying.resource, spec=AgentSpec(harness="claude-code", name="g", model="m"),
                                 ...
                                 model_service_account="harness-run-model@p.iam.gserviceaccount.com")
...
def test_scripted_worker_pool_end_to_end_with_control_ready():
    provider = FakeSandboxProvider(lambda n: ScriptedWorker(auto=[
        __import__("harness_run").events.AgentEvent(kind="status", summary="ready", raw={"event": "control_ready"}),
        result_event("42"),
    ]))
    engine = make_engine(provider, warm=True)
    engine.fill_pool(1)
