...
from harness_run.checkpoint import restore, snapshot
...
def test_try_restore_reports_the_failure_instead_of_swallowing_it(tmp_path, caplog) -> None:
    from harness_run.checkpoint.workspace import try_restore, workspace_ready_event

    store = LocalBlobStore(str(tmp_path / "store"))
    snapshot(store, "sess-1", str(tmp_path))
    with caplog.at_level("WARNING", logger="harness_run.checkpoint.workspace"):
